...
// CMapLoadable::LoadMap is also used by this client for the login background.
// Arm weather processing only for the next map load initiated by SET_FIELD.
void Weather_ExpectGameplayLoad();

// Object WZ properties are needed only while RestoreObj has armed weather capture.
// Keeping this check outside lamps.cpp prevents login-stage objects from being probed.
