...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call,
  ...
  case DLL_PROCESS_ATTACH: {
    ...
    if (reader.ParseError() == 0) {
      ...
      Client::CustomLoginFrame =
          reader.GetBoolean("general", "CustomLoginFrame", true);
      ...
      Memory::UseVirtuProtect =
          reader.GetBoolean("general", "UseVirtuProtect", true);
      ...
      Client::bigLoginFrame =
          reader.GetBoolean("general", "bigLoginFrame", false);
      Client::SwitchChinese =
          reader.GetBoolean("general", "SwitchChinese", false);
      Client::speedMovementCap =
          reader.GetInteger("optional", "speedMovementCap", 140);
      ...
      ownCashShopFrame =
          reader.GetBoolean("optional", "ownCashShopFrame", false);
      EzorsiaV2WzIncluded =
          reader.GetBoolean("general", "EzorsiaV2WzIncluded", true);
      Client::ServerIP_AddressFromINI =
          reader.Get("general", "ServerIP_Address", "127.0.0.1");
      Client::serverIP_Port =
          reader.GetInteger("general", "serverIP_Port", 8484);
      Client::climbSpeedAuto =
          reader.GetBoolean("optional", "climbSpeedAuto", false);
      ...
      Client::fullScreenPetLooting =
          reader.GetBoolean("optional", "fullScreenPetLooting", false);
      Client::closeRangeShooting =
          reader.GetBoolean("optional", "closeRangeShooting", false);
      Client::dropCashItem =
          reader.GetBoolean("optional", "dropCashItem", false);
      Client::ignoreGender =
          reader.GetBoolean("optional", "ignoreGender", false);
    }
    ...
    HookCWvsApp__InitializeResMan(
        false); // experimental //ty to all the contributors of the ragezone
                // release: Client load .img instead of .wz v62~v92
    Hook_StringPool__GetString(
        true); // hook stringpool modification //ty !! popcorn //ty darter
    Hook_lpfn_NextLevel(true);
    // Hook_get_unknown(true);
    // Hook_get_resource_object(true); //helper function hooks  //ty teto for
    // helping me get started Hook_com_ptr_t_IWzProperty__ctor(true);
    // Hook_com_ptr_t_IWzProperty__dtor(true);

    Client::UpdateGameStartup();

    std::cout << "Applying resolution " << Client::m_nGameWidth << "x"
              << Client::m_nGameHeight << std::endl;
