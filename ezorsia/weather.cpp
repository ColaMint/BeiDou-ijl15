...
static std::atomic<bool>          g_bExpectedGameplayLoad{false};
...
void Weather_ExpectGameplayLoad() {
    g_bExpectedGameplayLoad.store(true);
}

bool Weather_ShouldInspectMapObject() {
    return g_bCapture && g_bCaptureObj;
}
...
void Weather_HandleWorldState(CInPacket* pPacket) {
    ...
    const size_t uEntryOffset = pPacket->GetOffset();
    pPacket->SetOffset(uEntryOffset + 2);
        ...
        try {
            ...
            if (Lamps_CaptureLayer(*pResult)) {
                ...
                if (g_bSwayLeafreBack) {
                    // WeatherSway_ReplaceLeafreBack(*pResult);  // Sway temporarily disabled.
                }
            } else if (g_bCapture) {
                if (g_bCaptureObj) {
...
void CMapLoadable::Update_hook() {
    ...
    ApplyBareSky();
    // SweepFoliageSway();  // Fallback translation sway temporarily disabled.
